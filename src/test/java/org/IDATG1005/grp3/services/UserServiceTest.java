...
class UserServiceTest {
  ...
  void createUser_Success() throws Exception {
    ...
    when(userDaoMock.createUser(anyString(), eq(email), eq(password))).thenReturn(expectedUser);

    User resultUser = userService.createUser(username, email, password);
    ...
    verify(userDaoMock, times(1)).createUser(username, email, password);
  }
  ...
  void createUser_ThrowsUsernameAlreadyExistsException() throws Exception {
    ...
    when(userDaoMock.createUser(anyString(), anyString(), anyString())).thenThrow(new UsernameAlreadyExistsException(username));
  ...
  void updateUserDetails_Success() throws Exception {
    User userToUpdate = new User(1, "updatedUser", "update@example.com", "newPassword123");
