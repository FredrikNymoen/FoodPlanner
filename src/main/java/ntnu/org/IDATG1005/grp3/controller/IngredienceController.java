...
public class IngredienceController implements Initializable, EditBoxDisplayListener,
  ...
  private final Map<InventoryIngredient, AnchorPane> ingredientUIMap = new HashMap<>();
  ...
  private void displayIngredients() {
    ...
    if (favoriteCheckBox.isSelected()) {
      ...
      Collection<InventoryIngredient> inventoryIngredients = appUser.getInventory().getIngredients()
          .values();
  ...
  /**
   * Setup a global click listener to close the search results when clicking outside the search
   * field
   */
  private void setupGlobalClickListener() {
    rootPane.addEventFilter(MouseEvent.MOUSE_CLICKED, event -> {
      ...
      if (isNodeOrParentInstanceOf(target, TextField.class) || isDescendantOrSelfWithId(target,
          "searchAlternative")) {
      } else {
        ingredienceSearchGrid.getChildren().clear();
